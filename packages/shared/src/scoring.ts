import type { ActivityItem, ItemScore, ScoringWeights } from "./types";
...
function scoreDelivery(item: Omit<ActivityItem, "score" | "summary">, weights: ScoringWeights): number {
  const params = weights.deliveryParams;
  const linkedBonus = Math.min(20, item.linkedItems.length * params.linkedBonus);
  const diffBonus =
    item.metrics.diffSize && item.metrics.diffSize > 0
...
function scoreEngagement(item: Omit<ActivityItem, "score" | "summary">, weights: ScoringWeights): number {
  const params = weights.engagementParams;
  const base =
    item.metrics.commentsCount * params.commentWeight +
